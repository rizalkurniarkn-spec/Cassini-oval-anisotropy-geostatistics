...
CASSINI_B = 149.27069331
_CASSINI_INTERNAL_SCALE_X = 0.87479086
_CASSINI_INTERNAL_SCALE_Y = 1.14313029
...
def load_thickness_data():
    ...
    z_col = Z_COL if Z_COL in df.columns else detect_column(
        df, ["Saprolite Thickness", "saprolite thickness", "sap thickness", "thickness"]
    )
    ...
    if x_col is None or y_col is None or z_col is None:
        ...
        raise ValueError(
            "X, Y, or Saprolite Thickness column could not be detected. "
    ...
    if n_after < n_before:
        print(
            ...
            "were merged using the mean Saprolite Thickness."
        )
    ...
    print(f"Min Saprolite Thickness   = {data['Z'].min():.4f}")
    print(f"Max Saprolite Thickness   = {data['Z'].max():.4f}")
    print(f"Mean Saprolite Thickness  = {data['Z'].mean():.4f}")
    print(f"SD Saprolite Thickness   = {data['Z'].std(ddof=1):.4f}")
...
def build_orthogonal_boundary(coords, offset, cell=BOUNDARY_CELL):
    ...
    A regular 25 m raster with closing and hole filling creates a boundary
    that is orthogonal, clean, and free of internal holes.
    """
    coords = np.asarray(coords, dtype=float)
...
def plot_single_sgs_map(
    ...
    cbar.set_label("SGS Saprolite Thickness")

    ax.set_title(
        f"{model_name} - SGS Saprolite Thickness - Realization {realization_number:03d}",
        fontsize=13,
        fontweight="bold",
    )
    ax.set_xlabel("X")
...
def plot_comparison_sgs_map(
    ...
    if cf is not None:
        fig.colorbar(
            ...
            label="SGS Saprolite Thickness",
            shrink=0.88,
        )

    fig.suptitle(
        f"SGS Comparison Saprolite Thickness - Realization {realization_number:03d}",
        fontsize=13,
        fontweight="bold",
    )
    fig.savefig(output_file(file_name), dpi=MAP_DPI, bbox_inches="tight")
...
def plot_model_maps(
    ...
    suptitle,
    file_name,
    colorbar_label="Saprolite Thickness",
    fixed_vmin=None,
    fixed_vmax=None,
):
...
def plot_etype_differences(etype_maps, grid_info, hard_coords, file_name):
    ...
    fig.suptitle("Difference E-type SGS SAP: Cassini - Ellipse", fontsize=13, fontweight="bold")
...
def plot_recovered_directional_ranges(
    ...
    """Directly validate eight-direction ranges for Ellipse and Cassini."""
...
def relative_histogram_per_realization(arr2d, bins):
    """Percentage of data per bin for each realization; shape=(n_real, n_bins)."""
...
def plot_histogram_100_realizations_by_model(realizations, z_original):
    ...
    for model_name in MODELS:
        ...
        ax.set_xlabel("Saprolite Thickness")
...
def plot_uncertainty_ensemble(post, file_name="14_uncertainty_plot_100_realization_2model.png"):
    ...
    Grid nodes are ordered using the mean E-type of both models so that
    the x-axis in both panels refers to the same location ordering.
    """
    ...
    axes[0].set_ylabel("Saprolite Thickness")
...
def main():
    ...
    for r in range(N_REALIZATIONS):
        ...
        if SAVE_EACH_REALIZATION_CSV and SAVE_REALIZATION_CSV_DURING_SIMULATION:
            csv_path = DIR_REALIZATION_CSV / (
                f"R{realization_number:03d}_SGS_SAP_Ellipse_Cassini.csv"
            )
            pd.DataFrame({
                "X": valid_points[:, 0],
                "Y": valid_points[:, 1],
    ...
    if SAVE_ALL_REALIZATIONS:
        for model_name in MODELS:
            np.save(
                DIR_ARRAYS / f"SGS_SAP_{model_name}_{N_REALIZATIONS}realization.npy",
    ...
    for r in range(N_REALIZATIONS):
        ...
        if SAVE_EACH_REALIZATION_CSV and not SAVE_REALIZATION_CSV_DURING_SIMULATION:
            ...
            realization_df.to_csv(
                DIR_REALIZATION_CSV / f"R{real_no:03d}_SGS_SAP_Ellipse_Cassini.csv",
                index=False,
            )
        ...
        if SAVE_INDIVIDUAL_REALIZATION_MAPS:
            for model_name in MODELS:
                rel_file = Path("02_map_individual") / model_name / (
                    f"R{real_no:03d}_Map_SGS_SAP_{model_name}.png"
                )
    ...
    plot_model_maps(
        ...
        "SGS Saprolite Thickness - Example Paired Realization for Two Models",
    ...
    summary_specs = [
        ("Etype", "E-type", "E-type of 100 Realization SGS - Saprolite Thickness", "02_Etype_2model.png", "Saprolite Thickness"),
        ("SD", "SD", "Standard Deviation 100 Realization SGS - Saprolite Thickness", "03_SD_2model.png", "SD Saprolite Thickness"),
        ("Variance", "Variance", "Variance 100 Realization SGS - Saprolite Thickness", "04_Variance_2model.png", "Variance"),
        ("CL95_Lower", "P2.5", "Lower 95% Simulation Interval SGS - Saprolite Thickness", "05_P025_2model.png", "Saprolite Thickness"),
        ("CL95_Upper", "P97.5", "Upper 95% Simulation Interval SGS - Saprolite Thickness", "06_P975_2model.png", "Saprolite Thickness"),
        ("CL95_Width", "Width P2.5-P97.5", "Width 95% Simulation Interval SGS - Saprolite Thickness", "07_interval95_width_2model.png", "Interval width"),
        ("P25", "P25", "P25 of 100 Realization SGS - Saprolite Thickness", "08_P25_2model.png", "Saprolite Thickness"),
        ("P50", "P50", "P50 of 100 Realization SGS - Saprolite Thickness", "09_P50_2model.png", "Saprolite Thickness"),
        ("P75", "P75", "P75 of 100 Realization SGS - Saprolite Thickness", "10_P75_2model.png", "Saprolite Thickness"),
        ("P95", "P95", "P95 of 100 Realization SGS - Saprolite Thickness", "11_P95_2model.png", "Saprolite Thickness"),
    ]
    ...
    axes[0].set_xlabel("Mean Saprolite Thickness")
    axes[1].set_xlabel("SD Saprolite Thickness")
    ...
    print(f"4. One spatial-summary map set for both models")
    print(f"5. {N_REALIZATIONS} paired comparison maps for two models")
    print("6. E-type, SD, variance, simulation intervals, percentiles, and difference maps")
    print("7. Histogram: 100 realization curves per model (Y = relative frequency)")
    print("8. Uncertainty plot 100 realization")
    print("9. CDF: 100 realization curves in one figure for each model")
    print("10. Variogram: 100 realization curves in one figure for each model")
    print("11. Eight-direction variograms: two figures (Ellipse, Cassini)")
    print("12. Table of recovered directional ranges and variogram RMSE")
    print("13. Recovered eight-direction range plot (input vs model vs simulation)")
