# ============================================================
# SEQUENTIAL GAUSSIAN SIMULATION (SGS) OF SAPROLITE THICKNESS
# COMPARISON OF ELLIPSE VS CASSINI OVAL
# ============================================================
#
# Input:
#   Ketebalan.xlsx
#   Kolom variabel: "Ketebalan SAP"
#   Kolom koordinat X/Y dideteksi otomatis (X/Easting dan Y/Northing).
#
# Konsep:
#   1. Normal-score transform Ketebalan SAP.
#   2. SGS pada ruang Gaussian menggunakan Simple Kriging (mean = 0).
#   3. Both models use the same radial variogram parameters:
#        nugget = 0.06
#        partial sill = 0.94
#        total sill = 1.00
#        model = spherical
...
#        - Ellipse
#        - Affine-scaled Cassini Oval
#   5. 100 realizations use paired random paths and paired Gaussian
...
EXPERIMENTAL_VARIANCE_SCREENSHOT = 0.9986
...
# ============================================================
# Folder root sengaja berbeda agar hasil run SAP sebelumnya
# tidak tercampur/tertimpa. Di dalamnya output dipisahkan per jenis.
OUTPUT_DIR = REPO_ROOT / "outputs" / "sgs_saprolite"
...
# langsung dengan variogram target SGS (nugget=0.73, partial sill=0.27).
...
# Colormap sama dengan script estimasi SAP yang diberikan.
...
# ============================================================
# 2. PARAMETER DIRECTIONAL RANGE SAP - HASIL FITTING NLS TERBARU
# ============================================================


# Elips polar NLS
ELLIPSE_MAJOR = 174.31140386
ELLIPSE_MINOR = 117.81043609
ELLIPSE_PHI_AZ = 68.73013298
ELLIPSE_RATIO = ELLIPSE_MAJOR / ELLIPSE_MINOR

# Oval Cassini affine-scaled NLS
CASSINI_A = 116.61772890
CASSINI_C = 149.27069331
CASSINI_SX = 0.87479086
CASSINI_SY = 1.14313029
CASSINI_PHI_AZ = 64.94038898
...
DIRECTIONAL_VALIDATION_TABLE = pd.DataFrame({
    ...
    "Original_Range_m": [137.0, 140.0, 155.0, 172.0, 165.0, 150.0, 105.0, 111.0],
})
...
def load_thickness_data():
    ...
    z_col = Z_COL if Z_COL in df.columns else detect_column(
        df, ["Ketebalan SAP", "ketebalan sap", "sap thickness", "ketebalan"]
    )
    ...
    if x_col is None or y_col is None or z_col is None:
        ...
        raise ValueError(
            "Kolom X, Y, atau Ketebalan SAP tidak terdeteksi. "
    ...
    if n_after < n_before:
        print(
            ...
            "digabung menggunakan mean Ketebalan SAP."
        )
    ...
    print(f"Min Ketebalan SAP   = {data['Z'].min():.4f}")
    print(f"Max Ketebalan SAP   = {data['Z'].max():.4f}")
    print(f"Mean Ketebalan SAP  = {data['Z'].mean():.4f}")
    print(f"Std Ketebalan SAP   = {data['Z'].std(ddof=1):.4f}")
...
def build_orthogonal_boundary(coords, offset, cell=BOUNDARY_CELL):
    """
    Boundary dari union kotak +/- offset di sekitar titik bor.
    Raster reguler 25 m + closing + fill-holes membuat boundary
    horizontal/vertikal, lebih rapi, dan tanpa lubang internal.
    """
    coords = np.asarray(coords, dtype=float)
...
def build_grid(data):
    ...
    boundary = build_orthogonal_boundary(
        coords, offset=boundary_buffer, cell=BOUNDARY_CELL
    )
...
def plot_single_sgs_map(
    ...
    cbar.set_label("SGS Ketebalan SAP")

    ax.set_title(
        f"{model_name} - SGS Ketebalan SAP - Realisasi {realization_number:03d}",
        fontsize=13,
        fontweight="bold",
    )
    ax.set_xlabel("X")
...
def plot_comparison_sgs_map(
    ...
    if cf is not None:
        fig.colorbar(
            ...
            label="SGS Ketebalan SAP",
            shrink=0.88,
        )

    fig.suptitle(
        f"Perbandingan SGS Ketebalan SAP - Realisasi {realization_number:03d}",
        fontsize=13,
        fontweight="bold",
    )
    fig.savefig(output_file(file_name), dpi=MAP_DPI, bbox_inches="tight")
...
def plot_etype_differences(etype_maps, grid_info, hard_coords, file_name):
    ...
    fig.suptitle("Perbedaan E-type SGS SAP: Cassini - Elips", fontsize=13, fontweight="bold")
...
def plot_recovered_directional_ranges(
    ...
    """Validasi langsung directional range 8 arah untuk Elips dan Cassini."""
...
def relative_histogram_per_realization(arr2d, bins):
    """Persentase data per bin untuk setiap realisasi; shape=(n_real, n_bins)."""
    arr2d = np.asarray(arr2d, dtype=float)
    out = np.empty((arr2d.shape[0], len(bins) - 1), dtype=float)
    for r in range(arr2d.shape[0]):
        vals = arr2d[r]
        vals = vals[np.isfinite(vals)]
        counts, _ = np.histogram(vals, bins=bins)
        out[r] = 100.0 * counts / max(1, counts.sum())
    return out
...
def plot_uncertainty_ensemble(post, file_name="14_uncertainty_plot_100_realisasi_2model.png"):
    ...
    Node grid diurutkan dengan referensi E-type rata-rata kedua model agar
    sumbu-x pada kedua panel menunjuk urutan lokasi yang sama.
    """
    ...
    axes[0].set_ylabel("Ketebalan SAP")
...
def plot_cdf_100_realizations_by_model(realizations, z_original):
    ...
    for model_name in MODELS:
        ...
        for r in range(arr.shape[0]):
            ...
            curves.append(cdf)
            ax.plot(
                x, cdf, linewidth=0.65, alpha=0.15,
                label="100 realisasi" if r == 0 else None,
            )
...
def plot_variogram_100_realizations_by_model(variogram_curves, pair_info):
    ...
    for model_name in MODELS:
        ...
        for r in range(curves.shape[0]):
            ax.plot(
                centers, curves[r], linewidth=0.7, alpha=0.16,
                label="100 variogram realisasi" if r == 0 else None,
            )

        ax.plot(
            centers, mean_curve, linewidth=2.4, marker="o", markersize=3,
            label="Mean 100 realisasi",
        )
        ax.plot(
            centers, median_curve, linewidth=1.8, linestyle="--",
            label="Median 100 realisasi",
        )
        ax.plot(
            centers, target, linewidth=2.2, linestyle=":",
...
def main():
    ...
    for r in range(N_REALIZATIONS):
        ...
        # Simpan paired CSV segera setelah triplet model pada realisasi ini selesai.
        ...
        if SAVE_EACH_REALIZATION_CSV and SAVE_REALIZATION_CSV_DURING_SIMULATION:
            csv_path = DIR_REALIZATION_CSV / (
                f"R{realization_number:03d}_SGS_SAP_Elips_Cassini.csv"
            )
            pd.DataFrame({
                "X": valid_points[:, 0],
                "Y": valid_points[:, 1],
    ...
    if SAVE_ALL_REALIZATIONS:
        for model_name in MODELS:
            np.save(
                DIR_ARRAYS / f"SGS_SAP_{model_name}_{N_REALIZATIONS}realisasi.npy",
                realizations[model_name],
            )
        np.savez(
            DIR_ARRAYS / "grid_metadata_SGS_SAP.npz",
    ...
    if SAVE_POSTPROCESS_CSV:
        for model_name in MODELS:
            ...
            out.to_csv(DIR_TABLES / f"postprocess_SAP_{model_name}.csv", index=False)

    pd.DataFrame({
        "NormalScore_table": y_table,
        "Ketebalan_SAP_sorted": z_table,
    ...
    # Skala global sama untuk SELURUH peta yang bersatuan ketebalan SAP.
    ...
    for r in range(N_REALIZATIONS):
        ...
        if SAVE_EACH_REALIZATION_CSV and not SAVE_REALIZATION_CSV_DURING_SIMULATION:
            ...
            realization_df.to_csv(
                DIR_REALIZATION_CSV / f"R{real_no:03d}_SGS_SAP_Elips_Cassini.csv",
                index=False,
            )
        ...
        if SAVE_INDIVIDUAL_REALIZATION_MAPS:
            for model_name in MODELS:
                rel_file = Path("02_peta_individual") / model_name / (
                    f"R{real_no:03d}_Peta_SGS_SAP_{model_name}.png"
                )
    ...
    plot_model_maps(
        ...
        "SGS Ketebalan SAP - Contoh Paired Realization 2 Model",
    ...
    summary_specs = [
        ("Etype", "E-type", "E-type dari 100 Realisasi SGS - Ketebalan SAP", "02_Etype_2model.png", "Ketebalan SAP"),
        ("SD", "SD", "Standard Deviation 100 Realisasi SGS - Ketebalan SAP", "03_SD_2model.png", "SD Ketebalan SAP"),
        ("Variance", "Variance", "Variance 100 Realisasi SGS - Ketebalan SAP", "04_Variance_2model.png", "Variance"),
        ("CL95_Lower", "P2.5", "Lower 95% Simulation Interval SGS - Ketebalan SAP", "05_P025_2model.png", "Ketebalan SAP"),
        ("CL95_Upper", "P97.5", "Upper 95% Simulation Interval SGS - Ketebalan SAP", "06_P975_2model.png", "Ketebalan SAP"),
        ("CL95_Width", "Width P2.5-P97.5", "Lebar 95% Simulation Interval SGS - Ketebalan SAP", "07_interval95_width_2model.png", "Lebar interval"),
        ("P25", "P25", "P25 dari 100 Realisasi SGS - Ketebalan SAP", "08_P25_2model.png", "Ketebalan SAP"),
        ("P50", "P50", "P50 dari 100 Realisasi SGS - Ketebalan SAP", "09_P50_2model.png", "Ketebalan SAP"),
        ("P75", "P75", "P75 dari 100 Realisasi SGS - Ketebalan SAP", "10_P75_2model.png", "Ketebalan SAP"),
        ("P95", "P95", "P95 dari 100 Realisasi SGS - Ketebalan SAP", "11_P95_2model.png", "Ketebalan SAP"),
    ]
    ...
    axes[0].set_xlabel("Mean Ketebalan SAP")
    axes[1].set_xlabel("Std Ketebalan SAP")
    ...
    print(f"4. 1 set peta ringkasan 2 model")
    print(f"5. {N_REALIZATIONS} peta perbandingan paired 2 model")
    print("6. E-type, SD, variance, interval simulasi, percentile, dan difference maps")
    print("7. Histogram halus: 100 garis realisasi per model (Y = Frekuensi relatif)")
    print("8. Uncertainty plot 100 realisasi")
    print("9. CDF: 100 garis realisasi dalam 1 gambar untuk tiap model")
    print("10. Variogram: 100 garis realisasi dalam 1 gambar untuk tiap model")
    print("11. Variogram directional 8 arah: 2 gambar (Elips, Cassini)")
    print("12. Tabel recovered directional range dan RMSE variogram")
    print("13. Plot recovered directional range 8 arah (input vs model vs simulasi)")
