...
required = {"X", "Y", "Limonite Thickness", "Saprolite Thickness"}
...
# 3) Run the Cassini fitting interface and verify the published a and b parameters.
cassini_predict, cassini_params = sap_nls.fit_cassini(
    sap_nls.AZIMUTH_DEG, sap_nls.OBSERVED_RANGE_M
)
cassini_pred = cassini_predict(sap_nls.AZIMUTH_DEG)
assert np.all(np.isfinite(cassini_pred)) and np.all(cassini_pred > 0)
assert np.isfinite(cassini_params["Cassini_a_m"])
assert np.isfinite(cassini_params["Cassini_b_m"])

# 4) Evaluate the Cassini implicit equation at public test coordinates.
...
print(f"Cassini a/b: {cassini_params['Cassini_a_m']:.3f} / {cassini_params['Cassini_b_m']:.3f} m")
